  ...
  return true; 
		...
		<!--Body content-->
			...
			<div>
					...
					if($_POST['action']=="edit_info")
					{
						...
						if(isset($_POST['nom']) && isset($_POST['prenom']) && isset($_POST['username']) && isset($_POST['email']) )
						{
							...
							if(empty($errors))
							{
									...
									if($res == TRUE )
									{
										$sql='UPDATE `enseignant` SET `email`="'.$_POST['email'].'",`nom`="'.$_POST['nom'].'",`prenom`="'.$_POST['prenom'].'" WHERE `enseignantID`='.$prof_id;
										$res = $bdd->query($sql);
				...
				{
					$sql='SELECT `nom`, `prenom` FROM `enseignant` WHERE `enseignantID`='.$prof_id;
					$res = $bdd->query($sql);
					if($res== TRUE || $res->num_rows >0)
					{
          ...
          <div class="row-fluid">
            <div class="span12">
                <div class="well well-small">
                    <h3><i style="vertical-align: middle;margin-bottom: 5px;" class="icon icon-user"></i> Mon Profil</h3>
                </div>
                
                            <div > 
                              <ul class="nav nav-tabs">
                                <li class="<?PHP if(empty($success_pass)) echo 'active';?>"><a href="#profil" data-toggle="tab">Profil</a></li>
                                <li <?PHP if(!empty($success_pass)) echo 'class="active"';?>><a href="#password" data-toggle="tab">Mot de passe</a></li>
                              </ul>
                              <div class="tab-content">
                                   <div class="tab-pane <?PHP if(empty($success_pass)) echo 'active';?>" id="profil"><!-- Profil -->
                                        <div class="row-fluid">
                                            <div class="span10">
										<?php
