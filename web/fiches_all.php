...
if(login_check($bdd) == true) {
    ...
    <div>
        <ul class="breadcrumb">
            ...
            <li >
                <a href="fiches.php"> Fiches </a><span class="divider">/</span>
            </li>
    ...
    <div class="well">
        ...
        <span class="pull-right">
                          ...
                          <a class="toggle-vis" data-column="1"><span class="label label-info">Status</span></a>
                          <a class="toggle-vis" data-column="2"><span class="label label-info">Module</span></a>
                          <a class="toggle-vis" data-column="3"><span class="label label-info">Element</span></a>
                          <a class="toggle-vis" data-column="4"><span class="label label-info">Filère</span></a>
                          <a class="toggle-vis" data-column="5"><span class="label label-info">Semestre</span></a>
                          <a class="toggle-vis" data-column="6"><span class="label label-info">Choix</span></a>
        </span>
    </div>
<?php
}
        ...
        if($fiche_active==-1)
        {
            ...
            if($prof == TRUE) // from above
                while ($row = $prof->fetch_assoc())
                {
                    ...
                    $sql='SELECT `module_DetailsID`, module.designation AS mod_des, module.semestre, F.designation AS filiere FROM `module_details` INNER JOIN module ON module_details.moduleID = module.moduleID LEFT JOIN filiere AS F ON F.filiereId=module.filiereID WHERE `periode`=1 AND `annee_UniversitaireID`='.$CurYear;
                    //    echo $sql;die();
                    $res = $bdd->query($sql);
                    if($res == TRUE && $res->num_rows >0)
                        while($row = $res->fetch_assoc())
                        {
...

                            {
                                ...
                                $sql="SELECT DISTINCT `element_Module_DetailsID`, EMD.`element_ModuleID`, `designation`  FROM `element_module_details` AS `EMD` LEFT JOIN `element_module` AS `EM` ON `EMD`.`element_ModuleID` = `EM`.`element_ModuleID` WHERE `module_DetailsID`=".$row['module_DetailsID']." AND element_Module_DetailsID IN (SELECT `element_Module_DetailsID` FROM `fiche_souhait_details` WHERE `fiche`=".$fiche['id']." AND fiche_souhait_details.`groups`!=0 AND `enseignantID`=".$id." ) ";
                                //   echo $sql; die();
                                $res1 = $bdd->query($sql);
							...
							FROM (((fiche_souhait_details INNER JOIN element_module_details ON fiche_souhait_details.element_Module_DetailsID = element_module_details.element_Module_DetailsID) INNER JOIN element_module ON element_module_details.element_ModuleID = element_module.element_ModuleID) INNER JOIN module_details ON element_module_details.module_DetailsID = module_details.module_DetailsID)  WHERE fiche_souhait_details.`element_Module_DetailsID`=".$row1['element_Module_DetailsID']." AND fiche_souhait_details.`fiche`=".$fiche['id']." AND fiche_souhait_details.nature=\"cours\" AND fiche_souhait_details.`groups`!=0 AND fiche_souhait_details.`enseignantID`=".$id;
                                        ...
                                        if($res2 == TRUE && $res2->num_rows >0)
                                        {
                                            ...
                                            if($row2 = $res2->fetch_assoc())
                                            {
                                                ...
                                                echo get_label($row2['groups'],1,"Cours").'    ';
                                            }
                                        }

                                        //TD
                                        $sql="SELECT fiche_souhait_details.groups, fiche_souhait_details.nature, fiche_souhait_details.id, element_module_details.grp_tp, element_module_details.grp_td, element_module.designation, element_module.heures_cours, element_module.heures_td, element_module.heures_tp
							FROM (((fiche_souhait_details INNER JOIN element_module_details ON fiche_souhait_details.element_Module_DetailsID = element_module_details.element_Module_DetailsID) INNER JOIN element_module ON element_module_details.element_ModuleID = element_module.element_ModuleID) INNER JOIN module_details ON element_module_details.module_DetailsID = module_details.module_DetailsID)  WHERE fiche_souhait_details.`element_Module_DetailsID`=".$row1['element_Module_DetailsID']." AND fiche_souhait_details.`fiche`=".$fiche['id']." AND fiche_souhait_details.nature=\"TD\" AND fiche_souhait_details.`groups`!=0 AND fiche_souhait_details.`enseignantID`=".$id;
                                        ...
                                        //TP
                                        $sql="SELECT fiche_souhait_details.groups, fiche_souhait_details.nature, fiche_souhait_details.id, element_module_details.grp_tp, element_module_details.grp_td, element_module.designation, element_module.heures_cours, element_module.heures_td, element_module.heures_tp
							FROM (((fiche_souhait_details INNER JOIN element_module_details ON fiche_souhait_details.element_Module_DetailsID = element_module_details.element_Module_DetailsID) INNER JOIN element_module ON element_module_details.element_ModuleID = element_module.element_ModuleID) INNER JOIN module_details ON element_module_details.module_DetailsID = module_details.module_DetailsID)  WHERE fiche_souhait_details.`element_Module_DetailsID`=".$row1['element_Module_DetailsID']." AND fiche_souhait_details.`fiche`=".$fiche['id']." AND fiche_souhait_details.nature=\"TP\" AND fiche_souhait_details.`groups`!=0 AND fiche_souhait_details.`enseignantID`=".$id;
